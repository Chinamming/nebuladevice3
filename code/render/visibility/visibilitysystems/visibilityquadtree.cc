...
VisibilityQuadtree::RenderCell(const Ptr<VisibilityCell>& cell, const Math::float4& color)
{
    ...
    if (cell->GetEntityContexts().Size() > 0)
    {
        ...
        for (i = 0; i < cell->GetEntityContexts().Size(); ++i)
        {
            ...
            {
            case InternalGraphicsEntityType::Model:
                u = 255;                
                break;
            case InternalGraphicsEntityType::Camera:                               
                v = 255;
                break;
            case InternalGraphicsEntityType::Light:                
                u -= 128;
                v -= 128;
                break;
            }
    ...
    ShapeRenderer::Instance()->AddShape(RenderShape(Thread::GetMyThreadId(), RenderShape::Box, cell->GetBoundingBox().to_matrix44(), curColor));    
...
{   
    ...
    JobDataDesc inputData(observer->GetTypeRef(), sizeof(uint), sizeof(uint), 
    ...
    // update output data
    JobDataDesc outputData(outEntitiyArray.Begin(), outEntitiyArray.Size() * sizeof(Ptr<VisibilityContext>), outEntitiyArray.Size() * sizeof(Ptr<VisibilityContext>));
    // setup job with data
