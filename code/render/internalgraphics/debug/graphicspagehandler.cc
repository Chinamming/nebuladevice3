...
GraphicsPageHandler::HandleRequest(const Ptr<HttpRequest>& request)
{
    ...
    htmlWriter->SetTitle("Nebula3 Internal Graphics Subsystem");
    if (htmlWriter->Open())
    {
        htmlWriter->Element(HtmlElement::Heading1, "Internal Graphics Subsystem");
        ...
        htmlWriter->Element(HtmlElement::Heading3, "InternalStages");
        ...
        htmlWriter->Element(HtmlElement::Heading3, "InternalViews");
        htmlWriter->AddAttr("border", "1");
        htmlWriter->AddAttr("rules", "cols");
        htmlWriter->Begin(HtmlElement::Table);
            htmlWriter->AddAttr("bgcolor", "lightsteelblue");
            htmlWriter->Begin(HtmlElement::TableRow);
                htmlWriter->Element(HtmlElement::TableHeader, "Name");
                htmlWriter->Element(HtmlElement::TableHeader, "InternalStage");
                ...
                htmlWriter->Begin(HtmlElement::TableRow);
                    ...
                    htmlWriter->Begin(HtmlElement::TableData);                        
                        if (view->GetRenderTarget().isvalid())
                        {
                            ...
                            htmlWriter->AddAttr("href", "/texture?texinfo=" + rtResId.AsString());
        ...
        htmlWriter->Element(HtmlElement::Heading3, "InternalEntities");
...
GraphicsPageHandler::WriteEntityInfo(const Ptr<HttpRequest>& request, InternalGraphicsEntity::Id entityId)
{
    ...
    if (server->HasEntity(entityId))
    {
        ...
        if (htmlWriter->Open())
        {
            htmlWriter->Element(HtmlElement::Heading1, "Internal Graphics Entity Info");
            htmlWriter->AddAttr("href", "/index.html");
            htmlWriter->Element(HtmlElement::Anchor, "Home");
            htmlWriter->LineBreak();
            htmlWriter->AddAttr("href", "/graphics");
...
*/
Ptr<InternalModelEntity>
GraphicsPageHandler::LookupCharacterModelEntityById(InternalGraphicsEntity::Id entityId)
{
    Ptr<InternalModelEntity> result;
    InternalGraphicsServer* server = InternalGraphicsServer::Instance();
    if (server->HasEntity(entityId))
    {
        const Ptr<InternalGraphicsEntity>& entity = server->GetEntityById(entityId);
...
GraphicsPageHandler::PlayAnim(const Ptr<HttpRequest>& request, const String& argString)
{
    ...
    Ptr<InternalModelEntity> modelEntity = this->LookupCharacterModelEntityById(entityId);
...
{         
    ...
    const Ptr<InternalGraphicsEntity>& gfxEntity = server->GetEntityById(tokens[0].AsInt());
    ...
    {   
        ...
        {
            // get node from index
            const Ptr<ModelInstance>& instance = gfxEntity.cast<InternalModelEntity>()->GetModelInstance();
...
GraphicsPageHandler::WriteStateNodeInfo(const Ptr<StateNodeInstance>& stateNodeInst, const Ptr<Http::HtmlPageWriter>& htmlWriter)
{
    ...
    htmlWriter->Begin(HtmlElement::Table);
        htmlWriter->Begin(HtmlElement::TableRow);
            ...
            htmlWriter->Element(HtmlElement::TableData, stateNode->GetShader().Value());
        htmlWriter->End(HtmlElement::TableRow);
    // TODO: rest of members...
    htmlWriter->End(HtmlElement::Table);
}
